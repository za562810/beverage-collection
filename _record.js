...
const historyGroup = document.querySelector(".historyGroup"); //取得要放html的位子
...
function renderHistoryList(historys) {
  ...
  editButton = document.querySelectorAll(".history");
  editButton.forEach((e, index) => {
    let editList = editButton[index];
    editList.addEventListener("click", (e) => {
      targetId = e.currentTarget.getAttribute("data-id");
      targetDialog = filtered.find((record) => record.id === targetId);
      itemIndex = filtered.findIndex((record) => record.id === targetId);
...
let filtered = historys.filter((e) => e.date.slice(0, 7) === firstSixChars);

//產生初始historylist並預設最新排序
renderHistoryList(
  filtered.sort((a, b) => {
    if (b.date > a.date) {
...
monthPicker.addEventListener("change", (e) => {
  const firstSixChars = e.target.value.slice(0, 7);
  filtered = historys.filter((e) => e.date.slice(0, 7) === firstSixChars);
  filtered.sort((a, b) => {
    if (b.date > a.date) {
      return 1;
    } else if (b.date < a.date) {
...
//
//篩選品牌
const selectElement = document.getElementById("brand-list");

//比對品牌名稱
function filterBrand() {
  const selectedIndex = selectElement.selectedIndex;
  const selectedBrand = selectElement.options[selectedIndex].value;

  if (selectedBrand === "All brands") {
    filtered = historys; // 無篩選時顯示全部資料
  } else {
    filtered = historys.filter((e) => e.brand === selectedBrand);
  }
  const firstSixChars = monthPicker.value.slice(0, 7);
  filtered = filtered.filter((e) => e.date.slice(0, 7) === firstSixChars);
  renderHistoryList(filtered);
}
...
const confirmBtn = dialogFrame.querySelector("#confirmBtn");
const cancelBtn = dialogFrame.querySelector(".cancel");
let targetDialog; //點擊到的local storage資料
let targetId; //點擊到的物件id
let itemIndex; //點擊到的list在local storage的index
...
function renderDialog(target) {
  ...
  if (target.image.dataURL && target.image.dataURL != "/img/noImage.svg") {
    ...
    imgHTML = `<img id="preview-image" src="" style="display: none" />`;
  }
...
const overlay = dialogFrame.querySelector(".overlay");
...
confirmDelete.addEventListener("click", (e) => {
  ...
  dialogFrame.close();
  //delete historyList
  const deleteItem = document.querySelector(`[data-id="${targetId}"]`);
  deleteItem.remove();

  //delete from local storage
  historys.splice(itemIndex, 1);
